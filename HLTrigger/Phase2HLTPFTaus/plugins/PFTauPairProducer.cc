...
{
  bool
  isHigherPt(const std::pair<reco::PFTauRef, double>& pfTau_wChargedIso1,
...
void PFTauPairProducer::produce(edm::Event& evt, const edm::EventSetup& es)
{
  ...
  for ( std::vector<std::pair<reco::PFTauRef, double>>::const_iterator leadPFTau_wChargedIso = pfTaus_wChargedIso_sorted.begin();
    ...
    if ( !((min_pfTau_pt_            == -1. ||           leadPFTau_wChargedIso->first->pt()   >= min_pfTau_pt_           ) && 
           ...
           (max_pfTau_sumChargedIso_ == -1. ||           leadPFTau_wChargedIso->second        <= max_pfTau_sumChargedIso_)) ) continue;
    for ( std::vector<std::pair<reco::PFTauRef, double>>::const_iterator subleadPFTau_wChargedIso = leadPFTau_wChargedIso + 1;
      ...
      if ( !((min_pfTau_pt_            == -1. ||           subleadPFTau_wChargedIso->first->pt()   >= min_pfTau_pt_           ) && 
             ...
             (max_pfTau_sumChargedIso_ == -1. ||           subleadPFTau_wChargedIso->second        <= max_pfTau_sumChargedIso_)) ) continue;
      double dR = reco::deltaR(leadPFTau_wChargedIso->first->p4(), subleadPFTau_wChargedIso->first->p4());
      if ( !((min_dR_ == -1. || dR >= min_dR_) &&
             (max_dR_ == -1. || dR <= max_dR_)) ) continue;
      ...
      if ( !(leadPFTau_leadTrack && subleadPFTau_leadTrack) ) continue;
      double dz = std::fabs(leadPFTau_leadTrack->vertex().z() - subleadPFTau_leadTrack->vertex().z());
      if ( !((min_dz_ == -1. || dz >= min_dz_) &&
             (max_dz_ == -1. || dz <= max_dz_)) ) continue;
